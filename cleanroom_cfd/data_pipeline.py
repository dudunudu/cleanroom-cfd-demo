...
def load_timeline_window(csv_path, table_window_minutes):
...
def build_machine_state(df_window, svg_width_m, svg_height_m, machine_radius_m=0.30):
    machine_df = df_window[
        df_window["people_or_machine"].astype(str).str.strip().str.lower() == "machine"
    ].copy()

    machine_groups = machine_df.groupby("static_cluster_id")

    persistent_machine_entities = []
    machine_entity_map = {}

    for cluster_id, grp in machine_groups:
        label = grp["canonical_label"].mode().iloc[0]
        ...
        machine_entity_map[cluster_id] = ent

    machine_temp_lookup = {}
    for ts, grp in machine_df.groupby("timestamp"):
        temp_map = {}
        for cluster_id, subgrp in grp.groupby("static_cluster_id"):
            if subgrp["temp_mean_c"].notna().any():
                temp_map[cluster_id] = float(subgrp["temp_mean_c"].mean())
...
def update_machine_entities_for_time(
    ...
    for cluster_id, ent in machine_entity_map.items():
        if cluster_id in temp_map:
            ent.is_active = True
            ent.temp_target = temp_map[cluster_id]
        else:
            ent.is_active = False
...
def build_human_entities_for_time(
    ...
    human_rows = rows[
        rows["canonical_label"].astype(str).str.strip().str.lower() == "person"
    ].copy()
